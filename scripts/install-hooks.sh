#!/usr/bin/env bash
#
# Zairakai NPM Dev Tools - Git Hooks Installer
...
HOOKS_SOURCE_DIR="${DEV_TOOLS_ROOT}/stubs/githooks"
    ...
    for hook in "${HOOKS[@]}"; do
        case "$hook" in
            ...
            prepare-commit-msg)
                ...
                ;;
            pre-commit)
                echo -e "  ${GREEN}✓${NC} ${hook} ${YELLOW}→${NC} Quality checks (ESLint, Prettier, Markdown)"
                ;;
            pre-push)
