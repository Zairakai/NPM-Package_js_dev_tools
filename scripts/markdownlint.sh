...
ensure_bin "$MARKDOWNLINT_BIN" "markdownlint-cli2" || exit 1
...
if [[ -z "$MARKDOWNLINT_CONFIG" ]]; then
    ...
    log_warning "  node_modules/@zairakai/js-dev-tools/config/.markdownlint.json (default)"
    exit 1
fi
...
if "$MARKDOWNLINT_BIN" "${MARKDOWNLINT_ARGS[@]}"; then
    if [[ "$FIX_MODE" == "true" ]]; then
